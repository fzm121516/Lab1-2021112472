...
def save_graph_json(graph, file_path):
    ...
    json_graph = {node: {neighbour: weight for neighbour, weight in neighbours.items()}
...
def load_graph_json(file_path):
    ...
    graph = {node: {neighbour: weight for neighbour, weight in neighbours.items()}
             for node, neighbours in json_graph.items()}
    return graph
...
def query_bridge_words(word1, word2):
    ...
    if word1 not in graph or word2 not in graph:
        ...
        return f"No " + ", ".join(['\"' + word + '\"' for word in missing_words]) + " in the graph!"

    # 寻找桥接词。桥接词是指从 word1 可以直接到达的单词，并且这些单词可以直接到达 word2。
    bridge_words = [node for node in graph[word1] if node in graph and word2 in graph[node]]
...
def generate_new_text(input_text):
    ...
    for i in range(1, len(words)):
        ...
        if words[i - 1] in graph and words[i] in graph:
            bridge_words = [node for node in graph[words[i - 1]] if node in graph and words[i] in graph[node]]
